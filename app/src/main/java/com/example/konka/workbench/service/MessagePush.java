...
*/
    ...
    public MessagePush(Context context, NotificationManager mymsg,MessageDBrow[] msg){
        this.context = context;
        this.my_message = mymsg;
        this.mMessageDBrows = msg;
...
*/
    ...
    public void PushMessage(){
        ...
        for (int i=0;i<this.NumOfMsg;i++){
            strings = mMessageDBrows[i].getRowStringArray(MessageDBrow.BASE_ARRAY);
            msgID = strings[0];
            projectname = strings[1];
            state = strings[2];
            type = mMessageDBrows[i].getRowMarkString(MessageDBrow.MARK_TYPE_SINGLE);
            ...
            switch (type.charAt(0)){
                ...
                case '3':{timebp = mMessageDBrows[i].getRowStringArray(MessageDBrow.TIME_BP_ARRAY)[Integer.parseInt(type.substring(1))-1];
                    BOMtime_message(messageid,projectname,timebp,type);break;}//3:BOM生效时间提醒通知
            }
            MessageDBHelper.updateBDrow(mMessageDBrows[i],"MsgPush","0");//设置消息已推送
